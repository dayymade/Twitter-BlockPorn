...
window.addEventListener("message",async function(e){
    if(e.data.type=='menu_command_special_list'){

        menu_command_special_list();
    }
    if(e.data.type=='change_logo_off'){

        change_logo(false);
    }
    if(e.data.type=='change_logo_on'){
...
// 拉黑方法
const menu_command_special_list = function () {
  if (window.confirm("加急名单里有一些爱主动拉黑人的诈骗犯，要一键屏蔽吗？")) {
    block_special_list()
  } else {
    console.log('user cancelled block special scammers')
  }
}


// 修改图标
const change_logo = function (ison) {
  if(ison){
